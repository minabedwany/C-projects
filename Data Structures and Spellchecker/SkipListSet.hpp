...
template <typename ElementType>
SkipListKey<ElementType>::SkipListKey(SkipListKind kind)
    : kind{kind}
{
}
...
template <typename ElementType>
ElementType SkipListKey<ElementType>::getelement() const
{
    return element;
}
...
struct Node
{
    ...
    Node<ElementType> * down = nullptr;
    Node<ElementType> * next = nullptr;

    Node()
        :key(SkipListKind::PosInf), down(nullptr), next(nullptr)
    {
    }
    
    Node(SkipListKind kind)
        :key(kind), down(nullptr), next(nullptr)
    {
    }
    
    Node(SkipListKind kind, Node<ElementType> * nextNode, ElementType element)
        :key(SkipListKey<ElementType>(kind, element)), down(nullptr), next(nextNode)
    {
    }

    Node(SkipListKind kind, Node<ElementType> * nextNode)
        :key(SkipListKey<ElementType>(kind)), next(nextNode)
    {
    }

    Node(SkipListKind kind, Node<ElementType> * nextNode, Node<ElementType> * downNode, ElementType element)
        :key(SkipListKey<ElementType>(kind, element)), down(downNode), next(nextNode)
    {
    }
...
private:
    ...
    int maxheight = 10;
    int shouldGoUp = 0;
    Node<ElementType> ** rows = nullptr;

    Node<ElementType> * ptr = nullptr;
    Node<ElementType> * tail = nullptr;
    Node<ElementType> * downn = nullptr;
    ...
    void makeRow(int num)
    {
        ...
        if(num == 0)
        {
            rows[num] = new Node<ElementType>();
            Node<ElementType> * temp = rows[num];
            rows[num] = new Node<ElementType>(SkipListKind::NegInf, temp);
        }
        else
        {
            rows[num] = new Node<ElementType>();
            Node<ElementType> * temp = rows[num];
            rows[num] = new Node<ElementType>(SkipListKind::NegInf, temp);
            rows[num]->down = rows[num-1];
        }
    }

    void resize()
    {
        int newheight = 2*maxheight;

        Node<ElementType> ** newSList = new Node<ElementType> * [newheight];
    ...
    void removeRow(Node<ElementType> * node)
    {
    ...
    void deleteSkipList(Node<ElementType> ** nodes, int ht)
    {
        if(nodes != nullptr)
        {
            for(int i = 0; i < ht; ++i)
    ...
    void copy(Node<ElementType> ** src, int ht)
    {
        Node<ElementType> ** newSList = new Node<ElementType> * [ht];
        for(int i = 0; i<ht; ++i)
            newSList[i] = nullptr;
        
        deleteSkipList(rows, levels);
        rows = newSList;
        makeRow(0);
        ...
        for(int i = 0; i<ht; ++i)
        {
          if(src[i] != nullptr)
          {
            ...
            while(ptr != nullptr)
            {
                Node<ElementType> * myptr = rows[levels];
                SkipListKey<ElementType> tempkey(SkipListKind::Normal, ptr->key.getelement());
                insert(myptr, tempkey, levels);
                ++mySize;
                ptr = ptr->next;
            }
          }
    ...
    void insert(Node<ElementType> * node, SkipListKey<ElementType> & insertkey, int lvl)
    {
        if(node->next->key < insertkey)
            insert(node->next, insertkey, lvl);
        if(insertkey < node->next->key && lvl > 0)
            insert(node->down, insertkey, lvl -1);
        if(lvl == 0 && node->key < insertkey && insertkey < node->next->key)
        {
            tail = node;
            temp = node->next;
            tail->next = new Node<ElementType>(SkipListKind::Normal, temp, insertkey.getelement());

          if(levelTester != nullptr)
          {
            shouldGoUp = levelTester->shouldOccupyNextLevel(insertkey.getelement());
            if(shouldGoUp)
                downn = tail->next;
          }
        }
        ...
        if(shouldGoUp)
        {
            if(node->key < insertkey && insertkey < node->next->key && lvl > 0)
            {
                temp = node->next;
                tail = node;
                tail->next = new Node<ElementType>(SkipListKind::Normal, temp, downn, insertkey.getelement());

                shouldGoUp = levelTester->shouldOccupyNextLevel(insertkey.getelement());
                downn = tail->next;
            }

            if(lvl == levels  && shouldGoUp)
            {
                int upperlvl = levels + 1;
                while(upperlvl >= 1 && shouldGoUp)
                {
                    makeRow(upperlvl);
                    ++levels;
                    temp = rows[upperlvl]->next;
                    tail = rows[upperlvl];
                    tail->next = new Node<ElementType>(SkipListKind::Normal, temp, downn, insertkey.getelement());
                    downn = tail->next;
    ...
    bool searchAllLevels(Node<ElementType> * nodes, SkipListKey<ElementType> & searchkey, int lvl) const
    {
        if(nodes->next->key == searchkey)
            return true;
        if(nodes->next->key < searchkey)
            return searchAllLevels(nodes->next, searchkey, lvl);
        if(searchkey < nodes->next->key && lvl > 0)
            return searchAllLevels(nodes->down, searchkey, lvl-1);
        return false;
    }

    bool searchOneLevel(Node<ElementType> * nodes, SkipListKey<ElementType> & searchkey, int lvl) const
    {
        if(nodes->next->key == searchkey)
            return true;
        if(nodes->next->key < searchkey)
...
{
    rows = new Node<ElementType> * [maxheight];
    for(int i = 0; i < maxheight; ++i)
        rows[i] = nullptr;
    makeRow(0);
}
...
{
    Node<ElementType> ** temprows = rows;
...
{
    Node<ElementType> * temp = rows[levels];
    SkipListKey<ElementType> tempkey(SkipListKind::Normal, element);
    insert(temp, tempkey, levels);
    ++mySize;
...
{
    Node<ElementType> * tempnodes = rows[levels];
    SkipListKey<ElementType> tempkey(SkipListKind::Normal, element);
    return searchAllLevels(tempnodes, tempkey, levels);
}
...
{
    Node<ElementType> * tempnodes = rows[level];
    int counter = 0;
    if(tempnodes == nullptr)
        return 0;
    while(tempnodes != nullptr)
    {
        ++counter;
...
bool SkipListSet<ElementType>::isElementOnLevel(const ElementType& element, unsigned int level) const
{
    ...
    if(tempnodes == nullptr)
        return false;
    return searchOneLevel(tempnodes, tempkey, level);
}
